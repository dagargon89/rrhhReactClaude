...
export default function EditConfigPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [configRes, typesRes, deptsRes] = await Promise.all([
          fetch(`/api/incident-configs/${params.id}`),
          fetch("/api/incident-types"),
          fetch("/api/departments"),
        ])

        if (configRes.ok && typesRes.ok && deptsRes.ok) {
          ...
          reset({
            ...
            thresholdValue: parseFloat(config.thresholdValue),
  ...
  const onSubmit = async (data: UpdateConfigFormData) => {
    setIsLoading(true)
    try {
      const submitData = {
        ...data,
      ...
      const response = await fetch(`/api/incident-configs/${params.id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        ...
        router.push(`/admin/incident-configs/${params.id}`)
        router.refresh()
      } else {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-4">
          ...
          <div>
            <h3 className="text-lg font-semibold">Configuración no encontrada</h3>
            <p className="text-sm text-muted-foreground">
              La configuración que buscas no existe o ha sido eliminada
            </p>
          </div>
          <Button asChild>
  ...
  const typeName = INCIDENT_TYPE_NAMES[configData.incidentType.name] || configData.incidentType.name
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" asChild>
          <Link href={`/admin/incident-configs/${params.id}`}>
        ...
        <div className="space-y-2">
          ...
          <p className="text-lg text-muted-foreground">
            Modificar configuración de {typeName}
          </p>
        </div>
      </div>
                ...
                <SelectContent>
                  {incidentTypes.map((type) => (
                    <SelectItem key={type.id} value={type.id}>
                      {INCIDENT_TYPE_NAMES[type.name] || type.name}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
