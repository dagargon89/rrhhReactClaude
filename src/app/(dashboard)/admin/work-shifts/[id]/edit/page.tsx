...
import { ArrowLeft, Save, Loader2, Clock, ToggleLeft, Timer, AlertCircle } from "lucide-react"
import Link from "next/link"
import { toast } from "sonner"
import { updateWorkShiftSchema, type UpdateWorkShiftInput, type WorkDay } from "@/lib/validations/workShift"
...
export default function EditWorkShiftPage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(false)
  const [loadingData, setLoadingData] = useState(true)
  const [workShiftData, setWorkShiftData] = useState<any>(null)
  const [workingHours, setWorkingHours] = useState<WorkDay[]>([])
  ...
  const {
    ...
    setValue,
    reset,
  } = useForm<UpdateWorkShiftInput>({
    resolver: zodResolver(updateWorkShiftSchema),
  })
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetch(`/api/work-shifts/${params.id}`)
  ...
  const onSubmit = async (data: UpdateWorkShiftInput) => {
    setIsLoading(true)
    try {
      const response = await fetch(`/api/work-shifts/${params.id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        ...
        router.push(`/admin/work-shifts/${params.id}`)
        router.refresh()
      } else {
        const errorData = await response.json()
        toast.error(errorData.error || "Error al actualizar turno")
      }
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-4">
          ...
          <div>
            <h3 className="text-lg font-semibold">Turno no encontrado</h3>
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" asChild>
          <Link href={`/admin/work-shifts/${params.id}`}>
        ...
        <div className="space-y-2">
          ...
          <p className="text-lg text-muted-foreground">
            Modificar información de {workShiftData.name}
          </p>
        </div>
      </div>
