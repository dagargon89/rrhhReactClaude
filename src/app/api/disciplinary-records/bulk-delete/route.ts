import { NextRequest, NextResponse } from "next/server"
import { getServerSession } from "next-auth"
import { authOptions } from "@/lib/auth"
import { prisma } from "@/lib/prisma"
import { z } from "zod"

const bulkDeleteSchema = z.object({
  ids: z.array(z.string()).min(1, "Debe proporcionar al menos un ID"),
})

export async function DELETE(request: NextRequest) {
  try {
    // Verificar autenticación
    const session = await getServerSession(authOptions)

    if (!session?.user || !session.user.isStaff) {
      return NextResponse.json(
        { error: "No autorizado" },
        { status: 401 }
      )
    }

    // Parsear y validar el body
    const body = await request.json()
    const validation = bulkDeleteSchema.safeParse(body)

    if (!validation.success) {
      return NextResponse.json(
        { error: validation.error.errors[0].message },
        { status: 400 }
      )
    }

    const { ids } = validation.data

    // Verificar que los registros existen
    const records = await prisma.disciplinaryRecord.findMany({
      where: {
        id: {
          in: ids,
        },
      },
      select: {
        id: true,
        status: true,
      },
    })

    if (records.length === 0) {
      return NextResponse.json(
        { error: "No se encontraron registros para eliminar" },
        { status: 404 }
      )
    }

    // Verificar que no haya registros activos
    const activeRecords = records.filter(record => record.status === "ACTIVE")

    if (activeRecords.length > 0) {
      return NextResponse.json(
        { error: `No se pueden eliminar ${activeRecords.length} registro(s) activo(s)` },
        { status: 400 }
      )
    }

    // Eliminar los registros
    await prisma.disciplinaryRecord.deleteMany({
      where: {
        id: {
          in: ids,
        },
      },
    })

    return NextResponse.json({
      success: true,
      message: `${records.length} registro(s) eliminado(s) exitosamente`,
      deletedCount: records.length,
    })
  } catch (error) {
    console.error("Error en bulk delete de registros disciplinarios:", error)
    return NextResponse.json(
      { error: "Error al eliminar los registros" },
      { status: 500 }
    )
  }
}
